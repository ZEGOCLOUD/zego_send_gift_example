...
class _LivePageState extends State<LivePage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      subscriptions..add(
          ZegoUIKit().getSignalingPlugin().getInRoomTextMessageStream().listen(
              onInRoomTextMessageReceived))..add(
...
class GiftWidget extends StatefulWidget {
  const GiftWidget({Key? key, required this.onRemove, required this.giftPath}) : super(key: key);
  final VoidCallback onRemove;
  final String giftPath;

  static List<OverlayEntry> giftEntries = [];
  static bool clear() {
    for (final giftEntry in giftEntries) {
      if (giftEntry.mounted) {
        giftEntry.remove();
      }
    }
    giftEntries.clear();
  ...
  static void show(BuildContext context, String giftPath) {
    late OverlayEntry giftEntry;
    giftEntry = OverlayEntry(builder: (context) {
      return GiftWidget(onRemove: () {
        if (giftEntry.mounted) giftEntry.remove();
        giftEntries.remove(giftEntry);
      }, giftPath: giftPath,);
    });
    giftEntries.add(giftEntry);
    Overlay.of(context, rootOverlay: false)!.insert(giftEntry);
  }
